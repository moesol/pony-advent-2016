...
actor Main
  ...
  fun part1(input: String) =>
    ...
    for line in lines.values() do
      ...
      let checksum = compute_checksum(parts.slice(0, parts.size() - 1))
      (let sectorID', let checksum') = parse_last_part(try parts(parts.size() - 1) else "** error2" end)
      if checksum == checksum' then
        let sectorID = try sectorID'.read_int[USize]()._1 else 0 end
        sum = sum + sectorID
        let decrypted = decrypt_name(
          parts.slice(0, parts.size() - 1), sectorID
        )
        env.out.write("matches: ").print(sum.string())
        env.out.print("decrypted: " + decrypted)
      end
    end
  ...
  fun decrypt_name(parts: Array[String] box, sectorID: USize): String =>
    let result: String ref = String
    for part in parts.values() do
      for rune in part.runes() do
        let idx = (rune - 'a')
        let idx' = (idx + sectorID.u32()).mod(26)
        let char = 'a' + idx'
        result.push(char.u8())
      end
      result.push(' ')
    end
    result.append(sectorID.string())
    result.clone()

  fun compute_checksum(parts: Array[String] box): String =>
    let counts = Array[USize].init(0, 26) // indexed by character
    for part in parts.values() do
      for rune in part.runes() do
        let t = (rune - 'a')
        let idx: USize = (rune - 'a').usize()
        try
          counts(idx) = counts(idx) + 1
        else
          env.out.print("** error 3: " + idx.string())
        end
      end
    end
  ...
  fun parse_last_part(input: String): (String, String) =>
    try
      let r = Regex("(\\d+)\\[([a-z]+)\\]")
      let matched = r(input)
      (matched(1), matched(2))
